...
export async function getStaticPaths() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_EXTERNAL_RESSOURCES_URL}/api/audio-messages`);
...
export async function getStaticProps({ params }) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_EXTERNAL_RESSOURCES_URL}/api/audio-messages?q=${params.slug}`);
